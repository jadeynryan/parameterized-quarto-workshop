...
2. Variable names should be the first, and **only** the first, row of your data  
    - They should also adhere to variable name best practices
      ...
      - Don't start with a number or special character
...
These aren't just arbitrary practices. They serve a purpose.
    ...
    - They also make your variables more compatiable with languages such as R. For instance, R does not allow variable names to start with numbers. It will give you an error if you do this. Another example is that R also does not allow you to include dashes or hyphens in your variable names. They are consider subtraction and negation operators. So again you will get an error if you include those characters in your variable names.
...
3. Pick a format and stick to it
 ...
 - For instance if you could make a decision to always capture dates in this ISO 8601 standardized format. Using a standardized format like this has many benefits and can make your dates easier to work with. You can click on this date in the slides to learn more.
...
2. Because of the way dates are stored in R, they allow you to perform calculations using your dates, which is cool. You can add dates, subtract dates, etc. So as long as your dates are stored as dates, then we are good. However, if your dates are stored as character values, you will not be able to perform calculations on your dates.
...
![](images/type_clean.PNG){fig-align="center" width=90% height=90%}
...
2. Use consistent values to indicate missing responses
    - Blanks? NAs? -999?
...
2. So there are varying opinions on how missing data should be assigned. Some people think that missing data should be explicitly assigned with a value like -999. That way you know that the cell wasn't just skipped over by accident when data was being entered.
...
Ultimately I think what is important though, is that you use consistent values to represent missing data (choose one and stick with it), that your missing values are documented so that future users know how to interpret the values in your data, AND last, that your missing values match your variable type. If you use text to define missing values in a numeric variable, that variable will no longer be considered a numeric variable.
...
And by tool I mean whatever program you use to collect or enter data. So that could be Excel, RedCap, Qualtrics, or something else. Not all data entry and collection tools are created equal so make sure to consider the limitations and strengths of your tools. We don't have time to explore different data collection and entry tools today but if you want to learn more, you can watch this video from a previous R in Medicine workshop.
...
#| echo: false
#| cache: false
countdown(minutes = 1, font_size = "3em")
```
...
- are there any values that R should read in as NA? For instance, if we had added -99 as our missing values indicator, we could tell R to convert all of our -99s to NA when we read the data in
...
3. Are the values within your expected ranges? What outliers do you see? Is there a lack of variation? All values are equal to the same number?
...
But a quick word of caution about this function. You'll notice that it provides some confusing information for character variables. It gives a min and max value. This is actually the min and max character count for each variable.
