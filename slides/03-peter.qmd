...
- We have done DEV (Data Evaluation and Validation) - with more validation to be done, Data validation is a continuous process, right up to Data Lock.
...
- To make this work, we need one row per measurement of the outcome. This leads to long data, which look like this, with multiple visits and measurements for each patient.
...
- You will most often use long data, and this data structure allows you to look at multiple predictors and outcomes, like blood pressure, a PHQ-9 depression questionnaire, and a hemoglobin measurement (all collected on the same date).
...
- You may want to do analysis on both long and wide data in the same project. 
...
- Do This in [Posit Cloud](https://posit.cloud/content/5758558) 
...
::: columns
::: {.column width="60%"}
- Another data issue you may often face is in data collected over time, when one of two things happen:
...
emergency |> 
  ...
  mutate(overdoses = sum(str_detect(title, "OVERDOSE"))) |> 
...
- We want to know if the emergency department should order an extra case of Narcan each month in the summer months.
:::
:::
...
::: {.column width="60%"}
![Better Together: Chocolate and Peanut Butter](images/choc_pb.jpeg)
:::
...
- This is a *mutating* join - new variables are created/added to the LHS
...
[--\> Take me to the exercise \<--](https://shannonpileggi.github.io/rmedicine-data-cleaning-2023/exercises.html#ph2){target="_blank"}
